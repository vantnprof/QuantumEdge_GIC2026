...
Run: python -m quantumedge.pipelines.classical_baselines
"""
...
def run_pipeline(write_plots: bool = True) -> pd.DataFrame:
    settings = ensure_runtime_dirs()

    from quantumedge.data.loaders import load_all
    from quantumedge.features.builders import prepare_all
    from quantumedge.evaluation.metrics import compute_all
    ...
    # ------------------------------------------------------------------ #
    print("\n[SP500 RV — GARCH]")
    from quantumedge.models.garch import run_garch_on_financial
    ...
    print("\n[SP500 RV — HAR-RV]")
    from quantumedge.models.har_rv import run_har_rv
    ...
    from quantumedge.models.garch import run_garch_on_vix
    ...
    from quantumedge.models.hmm_regimes import fit_hmm, predict_regimes
    from quantumedge.evaluation.metrics import regime_accuracy, volatility_timing_sharpe
    ...
    csv_path = settings.results_dir / "metrics.csv"
    ...
    if write_plots:
        ...
        from quantumedge.visualization.plots import (
        ...
        _ts("5 / 6  Plot generation skipped")
    ...
    print(f"Plots saved to:    {settings.plots_dir}")
    print(f"Oxford-Man source: {data['oxford_source']}")
    print("\nNotes:")
...
def main(argv: list[str] | None = None) -> pd.DataFrame:
    parser = argparse.ArgumentParser(description="Run the QuantumEdge classical baseline pipeline.")
    parser.add_argument(
        "--skip-plots",
        action="store_true",
        help="Run data, models, and metrics without writing plot PNGs.",
    )
    args = parser.parse_args(argv)
    return run_pipeline(write_plots=not args.skip_plots)
